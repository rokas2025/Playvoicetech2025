...
export type TimingLog = {
  ...
  // Voice settings used
  llm_model?: string;
  tts_mode?: 'normal' | 'streaming-v1' | 'streaming-v2';
  interaction_mode?: 'push-to-talk' | 'conversational'; // NEW: Track interaction mode
  stability?: number;
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs">Režimas</th>
                <th className="px-2 py-2 text-left text-gray-700 font-semibold text-xs">TTS</th>
                ...
                <tr key={log.id} className="hover:bg-gray-50 text-xs">
                  ...
                  <td className="px-2 py-2 text-gray-700">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      log.interaction_mode === 'conversational'
                        ? 'bg-purple-100 text-purple-800'
                        : 'bg-orange-100 text-orange-800'
                    }`}>
                      {log.interaction_mode === 'conversational' ? '🎙️ Pokalbis' : '🎤 Push-to-Talk'}
                    </span>
                  </td>
                  <td className="px-2 py-2 text-gray-700">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      log.tts_mode === 'streaming-v2' 
                        ? 'bg-green-100 text-green-800' 
                        : log.tts_mode === 'streaming-v1'
                        ? 'bg-yellow-100 text-yellow-800'
                        : 'bg-gray-100 text-gray-800'
                    }`}>
                      {log.tts_mode === 'streaming-v2' ? '🚀 V2' : 
                       log.tts_mode === 'streaming-v1' ? '⚡ V1' : 
                       '📦 Normal'}
                    </span>
                  </td>
                  <td className="px-2 py-2 text-gray-700">
